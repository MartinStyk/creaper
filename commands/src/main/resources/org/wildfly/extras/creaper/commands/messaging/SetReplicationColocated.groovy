...
if (nn(excludedConnectors)) attrsArtemis['excluded-connectors'] = excludedConnectors

def defArtemis = { 'replication-colocated' (attrsArtemis) }

if (messagingHornetq) {
    throw new UnsupportedOperationException("HornetQ subsystem is not supported")
}
address = messagingActivemq.server
if ( address.'replication-colocated'.any { it.name() == 'replication-colocated' } ) {
    address.'replication-colocated'.replaceNode defArtemis
} else {
