 ...
 * @author mstyk
 */
public class AddRemoteConnectorOfflineTest {
    ...
    private static final String SUBSYSTEM_EXPECTED_REPLACE = ""
            ...
            + "                    </http-connector>\n"
            + "                    <in-vm-connector name=\"in-vm\" server-id=\"0\"/>\n"
            + "                    <netty-connector name=\"netty\" socket-binding=\"messaging1\"/>"
            + "                </connectors>\n"
            + "            </hornetq-server>\n"
            + "        </subsystem>\n"
            + "    </profile>\n"
            + "</server>";
    private static final String SUBSYTEM_ORIGINAL_WO_CONNECTORS = ""
            + "<server xmlns=\"urn:jboss:domain:1.7\">\n"
            + "    <profile>\n"
            + "        <subsystem xmlns=\"urn:jboss:domain:messaging:1.4\">"
            + "            <hornetq-server>\n"
            ...
            + "    </profile>\n"
            + "</server>";
    private static final String SUBSYTEM_TRANSFORM_WO_CONNECTORS = ""
            + "<server xmlns=\"urn:jboss:domain:1.7\">\n"
            + "    <profile>\n"
            + "        <subsystem xmlns=\"urn:jboss:domain:messaging:1.4\">"
            + "            <hornetq-server>\n"
    ...
    private static final String SUBSYSTEM_EXPECTED_REPLACE_ACTIVEMQ = ""
            ...
            + "                <in-vm-connector name=\"in-vm\" server-id=\"0\"/>\n"
            + "                <http-acceptor name=\"http-acceptor\" http-listener=\"default\"/>"
            + "                <remote-connector name=\"netty\" socket-binding=\"messaging1\" >\n"
            + "<param name=\"use-nio-global-worker-pool\" value=\"false\"/>\n"
            + "                </remote-connector>"
            + "            </server>\n"
            + "        </subsystem>\n"
            + "    </profile>\n"
    ...
    public void existing() throws Exception {
        ...
        client.apply(new AddRemoteConnector.Builder("netty")
                .socketBinding("http")
                .build());
    ...
    @Test
    public void transformWOConnectors() throws Exception {

        File cfg = tmp.newFile("xmlTransform.xml");
        Files.write(SUBSYTEM_ORIGINAL_WO_CONNECTORS, cfg, Charsets.UTF_8);

        OfflineManagementClient client = ManagementClient.offline(
                OfflineOptions.standalone().configurationFile(cfg).build());

        assertXmlIdentical(SUBSYTEM_ORIGINAL_WO_CONNECTORS, Files.toString(cfg, Charsets.UTF_8));

        client.apply(new AddRemoteConnector.Builder("netty")
                .socketBinding("messaging")
                .build());
        System.out.println(Files.toString(cfg, Charsets.UTF_8));
        assertXmlIdentical(SUBSYTEM_TRANSFORM_WO_CONNECTORS, Files.toString(cfg, Charsets.UTF_8));
    }
    ...
    public void replaceActiveMQ() throws Exception {
        Map<String, String> params = new HashMap<String, String>();
        params.put("use-nio-global-worker-pool", "false");
