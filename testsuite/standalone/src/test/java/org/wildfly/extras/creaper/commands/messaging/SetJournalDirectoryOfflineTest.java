 ...
 * @author mstyk
 */
public class SetJournalDirectoryOfflineTest {
    ...
    private static final String SUBSYTEM_EXPECTED_ACTIVEMQ = ""
            ...
            + "                <connection-factory name=\"InVmConnectionFactory\" connectors=\"in-vm\" entries=\"java:/ConnectionFactory\"/>\n"
            + "                <bindings-directory path=\"${sharedDirectory}/bindings\"/>\n"
            + "                <journal-directory path=\"${sharedDirectory}/journal\"/>"
            + "            </server>\n"
            + "        </subsystem>"
            + "    </profile>\n"
            + "</server>";
    private static final String SUBSYTEM_REPLACE_ACTIVEMQ = ""
            ...
            + "                <connection-factory name=\"InVmConnectionFactory\" connectors=\"in-vm\" entries=\"java:/ConnectionFactory\"/>\n"
            + "                <bindings-directory path=\"${sharedDirectory}/bindings\"/>\n"
            + "                <journal-directory path=\"${sharedDirectory}/journalREPLACED\"/>"
            + "            </server>\n"
            + "        </subsystem>"
            + "    </profile>\n"
            + "</server>";
    ...
    public void replace() throws Exception {
        ...
        client.apply(new SetJournalDirectory("${sharedDirectory}/journalREPLACED"));
    ...
    public void transform() throws Exception {
        ...
        client.apply(new SetJournalDirectory("${sharedDirectory}/journal"));
        System.out.println(Files.toString(cfg, Charsets.UTF_8));
        assertXmlIdentical(SUBSYTEM_EXPECTED_ACTIVEMQ, Files.toString(cfg, Charsets.UTF_8));
    }
