 ...
 * @author mstyk
 */
public class SetReplicationColocatedOfflineTest {
    ...
    private static final String SUBSYTEM_EXPECTED_ACTIVEMQ = ""
            ...
            + "                <jms-queue name=\"DLQ\" entries=\"java:/jms/queue/DLQ\"/>\n"
            + "                <connection-factory name=\"InVmConnectionFactory\" connectors=\"in-vm\" entries=\"java:/ConnectionFactory\"/>\n"
            + " <replication-colocated request-backup=\"true\" max-backups=\"1\" backup-port-offset=\"1000\"/>"
            + "            </server>\n"
            + "        </subsystem>"
            + "    </profile>\n"
            + "</server>";
    private static final String SUBSYTEM_REPLACE_ACTIVEMQ = ""
            ...
            + "                <jms-queue name=\"DLQ\" entries=\"java:/jms/queue/DLQ\"/>\n"
            + "                <connection-factory name=\"InVmConnectionFactory\" connectors=\"in-vm\" entries=\"java:/ConnectionFactory\"/>\n"
            + " <replication-colocated backup-port-offset=\"1000\" excluded-connectors=\"con1 con2\"/>"
            + "            </server>\n"
            + "        </subsystem>"
            + "    </profile>\n"
            + "</server>";
    ...
    public void replace() throws Exception {
        ...
        client.apply(new SetReplicationColocated.Builder()
                .backupPortOffset(1000)
                .excludedConnectors(cons)
                .build());
